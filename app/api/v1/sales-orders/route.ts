...
import { createSalesOrderDocument } from "@/lib/commercial/sales-orders";
...
export async function GET(request: Request) {
  ...
  let query = auth.context.supabase
    .from("sales_orders")
    .select("id, contact_id, sales_order_number, issue_date, due_date, total, status, currency, created_at, updated_at", { count: "exact" })
    .eq("org_id", auth.context.orgId);

  if (search) {
    query = query.ilike("sales_order_number", `%${search}%`);
  }
...
export async function POST(request: Request) {
  ...
  try {
    const { salesOrder } = await createSalesOrderDocument(auth.context, parsed.data);
    await audit(auth.context, "create", String(salesOrder.id), parsed.data as unknown as Json);
    return ok(salesOrder, undefined, { status: 201 });
  } catch (error) {
    return fail(400, { code: "CREATE_FAILED", message: error instanceof Error ? error.message : "Sales order could not be created." });
  }
}
