...
import { createQuotationDocument } from "@/lib/commercial/quotations";
...
export async function GET(request: Request) {
  ...
  let query = auth.context.supabase
    .from("quotations")
    .select("id, contact_id, quotation_number, issue_date, due_date, total, status, currency, created_at, updated_at", { count: "exact" })
...
export async function POST(request: Request) {
  ...
  try {
    const { quotation } = await createQuotationDocument(auth.context, parsed.data);
    await audit(auth.context, "create", String(quotation.id), parsed.data as unknown as Json);
    return ok(quotation, undefined, { status: 201 });
  } catch (error) {
    return fail(400, { code: "CREATE_FAILED", message: error instanceof Error ? error.message : "Quotation could not be created." });
  }
}
