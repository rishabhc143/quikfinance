...
import { createVendorCreditDocument } from "@/lib/commercial/adjustment-documents";
...
export async function GET(request: Request) {
  ...
  let query = auth.context.supabase
    .from("vendor_credits")
    .select("id, contact_id, bill_id, vendor_credit_number, issue_date, due_date, total, status, currency, created_at, updated_at", { count: "exact" })
    .eq("org_id", auth.context.orgId);

  if (search) {
    query = query.ilike("vendor_credit_number", `%${search}%`);
  }
  ...
  const rows = (data ?? []).map((row) => ({ ...row, vendor: contactMap.get(String(row.contact_id)) ?? "Vendor" }));
...
export async function POST(request: Request) {
  ...
  const parsed = vendorCreditSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const { vendorCredit } = await createVendorCreditDocument(auth.context, parsed.data);
    await audit(auth.context, "create", String(vendorCredit.id), parsed.data as unknown as Json);
    return ok(vendorCredit, undefined, { status: 201 });
  } catch (error) {
    return fail(400, { code: "CREATE_FAILED", message: error instanceof Error ? error.message : "Vendor credit could not be created." });
  }
}
