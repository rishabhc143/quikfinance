...
import { createCreditNoteDocument } from "@/lib/commercial/adjustment-documents";
...
export async function GET(request: Request) {
  ...
  let query = auth.context.supabase
    .from("credit_notes")
    .select("id, contact_id, invoice_id, credit_note_number, issue_date, due_date, total, status, currency, created_at, updated_at", { count: "exact" })
  ...
  const rows = (data ?? []).map((row) => ({ ...row, customer: contactMap.get(String(row.contact_id)) ?? "Customer" }));
...
export async function POST(request: Request) {
  ...
  try {
    const { creditNote } = await createCreditNoteDocument(auth.context, parsed.data);
    await audit(auth.context, "create", String(creditNote.id), parsed.data as unknown as Json);
    return ok(creditNote, undefined, { status: 201 });
  } catch (error) {
    return fail(400, { code: "CREATE_FAILED", message: error instanceof Error ? error.message : "Credit note could not be created." });
  }
}
